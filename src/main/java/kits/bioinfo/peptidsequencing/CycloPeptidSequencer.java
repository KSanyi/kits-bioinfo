...
public class CycloPeptidSequencer extends CycloPeptidSequencerBase {
	...
	public Set<Peptid> sequencePeptids(List<Integer> spectrum){
		...
		final Set<AminoAcid> candidateAminoAcids = aminoAcidCutRank < AminoAcid.values().length ?
				guessCandidateAminoAcids(experimentalSpectrum) : new HashSet<>(Arrays.asList(AminoAcid.values()));
