...
public class EulerianPathFinder {
	...
	private static <T> void checkGraph(Graph<T> graph) {
		if (graph.edges().isEmpty())
			throw new IllegalArgumentException("Can not find an Eulerian path in a graph without edges");
		...
		Set<T> startNodes = graph.nodes().stream().filter(node -> inDegreeMap.frequency(node) < outDegreeMap.frequency(node))
				.collect(Collectors.toSet());
		if (startNodes.size() > 1) {
			throw new IllegalArgumentException(
					"Can not find an Eulerian path in a graph where there is more than one node with indegree < outdegree as nodes: " + startNodes);
		}

		Set<T> endNodes = graph.nodes().stream().filter(node -> inDegreeMap.frequency(node) > outDegreeMap.frequency(node))
				.collect(Collectors.toSet());
		if (endNodes.size() > 1) {
			throw new IllegalArgumentException(
					"Can not find an Eulerian path in a graph where there is more than one node with indegree > outdegree as nodes: " + endNodes);
		}
	}
}
